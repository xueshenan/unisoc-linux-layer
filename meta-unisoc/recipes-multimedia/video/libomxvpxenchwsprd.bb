DESCRIPTION = "Unisoc codecs module - vp8 hw encoder"
...
PV = "0.1"
PR = "r0"
PROVIDES = "libomxvpxenchwsprd"

inherit get_release_mode

VCODEC_SRC_DIR = "${OEROOT}/source/unisoc/multimedia/sprd_codecs/vpx_mix/enc/hw/"
...
do_install_prepend() {
    if [ ${VCODEC_SRC_MODE} != "customer" ]; then
        rm -rf ${VCODEC_INSTALL_DIR}/libomx_vpxenc_hw_sprd.so
        install -d ${VCODEC_INSTALL_DIR}/
        install -m 0777 ${B}/libomx_vpxenc_hw_sprd.so ${VCODEC_INSTALL_DIR}/
    fi
}

do_install () {
    install -d ${D}${libdir}/
    install -m 0777 ${B}/libomx_vpxenc_hw_sprd.so ${D}${libdir}/
}

FILES_${PN} += "${libdir}/libomx_vpxenc_hw_sprd.so"
FILES_SOLIBSDEV = ""

DESCRIPTION = "Unisoc codecs module - h263/mpeg4 hw decoder"
...
PV = "0.1"
PR = "r0"
PROVIDES = "libomxm4vh263dechwsprd"

inherit get_release_mode

VCODEC_SRC_DIR = "${OEROOT}/source/unisoc/multimedia/sprd_codecs/m4v_h263_mix/dec/hw/"
...
do_install_prepend() {
    if [ ${VCODEC_SRC_MODE} != "customer" ]; then
        rm -rf ${VCODEC_INSTALL_DIR}/libomx_m4vh263dec_hw_sprd.so
        install -d ${VCODEC_INSTALL_DIR}/
        install -m 0777 ${B}/libomx_m4vh263dec_hw_sprd.so ${VCODEC_INSTALL_DIR}/
    fi
}

do_install () {
    install -d ${D}${libdir}/
    install -m 0777 ${B}/libomx_m4vh263dec_hw_sprd.so ${D}${libdir}/
}

FILES_${PN} += "${libdir}/libomx_m4vh263dec_hw_sprd.so"
FILES_SOLIBSDEV = ""

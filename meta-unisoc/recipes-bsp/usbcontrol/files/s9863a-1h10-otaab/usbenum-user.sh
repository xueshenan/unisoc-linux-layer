...
setln()
{
	debug=$(/usr/bin/rwini.sh /mnt/data/usbenum.ini $usbch debug)
...
usbch()
{
	echo "usbenum $usbch"
	if [ $usbch ];then
		idVendor=$(/usr/bin/rwini.sh /mnt/data/usbenum.ini $usbch idVendor)
		if [ $idVendor ];then
			DownUSB
			$(/usr/bin/rwini.sh /mnt/data/usbenum.ini property curr-usbch $usbch)
		else
			echo "no such USB mode"
			exit 1
		fi
		idProduct=$(/usr/bin/rwini.sh /mnt/data/usbenum.ini $usbch idProduct)
		configuration=$(/usr/bin/rwini.sh /mnt/data/usbenum.ini $usbch configuration)
		setid $idVendor $idProduct $configuration
		bcdDevice=$(/usr/bin/rwini.sh /mnt/data/usbenum.ini $usbch bcdDevice)
		bDeviceClass=$(/usr/bin/rwini.sh /mnt/data/usbenum.ini $usbch bDeviceClass)
		setdevice $bcdDevice $bDeviceClass
		manufacturer=$(/usr/bin/rwini.sh /mnt/data/usbenum.ini $usbch manufacturer)
		product=$(/usr/bin/rwini.sh /mnt/data/usbenum.ini $usbch product)
		setname $manufacturer $product
		MaxPower=$(/usr/bin/rwini.sh /mnt/data/usbenum.ini $usbch MaxPower)
		bmAttributes=$(/usr/bin/rwini.sh /mnt/data/usbenum.ini $usbch bmAttributes)
		setpower $MaxPower $bmAttributes
		rmlink
		VNIC=$(/usr/bin/rwini.sh /mnt/data/usbenum.ini $usbch VNIC)
		setVNIC $VNIC
		$(/usr/bin/rwini.sh /mnt/data/usbenum.ini property virtualcn $VNIC)
		for i in f1 f2 f3 f4 f5 f6 f7 f8 f9 f10
		do
			usbdir=$(/usr/bin/rwini.sh /mnt/data/usbenum.ini $usbch $i)
			setln $usbdir $i
		done
		if [ "$VNIC"x == "ums"x ];then
                        sleep 1
                        sdfile=$(/usr/bin/rwini.sh /mnt/data/usbenum.ini $usbch sdfile)
                        if [ -b $sdfile ];then

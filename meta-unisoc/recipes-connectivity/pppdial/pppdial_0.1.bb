...
EXTERNALSRC = "${EXTERNALSRC_DIR}/pppdial"
EXTERNALSRC_BUILD = "${EXTERNALSRC_DIR}/pppdial"
...
do_install_prepend () {
    if [ ${PPPDIAL_SRC_MODE} != "customer" ]; then
        rm -rf ${PPPDIAL_INSTALL_DIR}/pppdial

        install -d ${PPPDIAL_INSTALL_DIR}/pppdial
        install -m 0777 ${S}/pppdial ${PPPDIAL_INSTALL_DIR}/pppdial
    fi
}

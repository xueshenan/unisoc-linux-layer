...
UNISOC_EXTERNALSRC_USESSTATE ?= "1"

python () {
    # if inherit from externalsrc, then not enable unisoc_externalsrc
    if bb.data.inherits_class('externalsrc', d):
        bb.note("inherit from externalsrc!!!!")
        return
    ...
    usesstate = False
    if  d.getVar('UNISOC_EXTERNALSRC_USESSTATE') == '1':
        usesstate = True
    ...
    if not usesstate and (bpn == d.getVar('PN') or not classextend):
        if (externalsrc or
    ...
    if externalsrc:
        ...
        for task in tasks:
            if not usesstate and task.endswith("_setscene"):
...
unisoc_externalsrc_do_buildclean() {
